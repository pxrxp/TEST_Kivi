...
class ModelAdapter:
    ...
    def predict_sky_mask(self, image: np.ndarray) -> np.ndarray:
        """
        Run model inference and return binary sky mask.
        ...
        if self.backend_type == "onnx":
            ...
            input_channels = state_dict.get('encoder.conv1.weight', 
                                          state_dict.get('first.weight', 
                                                       torch.zeros(32, 3, 3, 3))).shape[1]
        ...
        if probability_map.shape != image.shape[:2]:
            probability_map = np.array(
                Image.fromarray((probability_map * 255).astype(np.uint8))
    ...
    def get_boundary_from_probability(self, prob_map: np.ndarray, 
